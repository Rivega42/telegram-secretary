...
### Безопасность (SaaS — ротация ключа шифрования)
- **Версионирование ключей** (`core/secrets-crypto.js`): `SECRETS_KEY` (основной) +
  `SECRETS_KEYS_OLD` (старые, через запятую). Дешифровка пробует все ключи (нужный
  определяется по auth-тегу GCM) — ротация без простоя
- **Перешифровка** `reencryptSecrets()` (admin `POST /api/admin/secrets/rotate`):
  перешифровывает все секреты основным ключом и пересчитывает слепые индексы.
  После прогона `SECRETS_KEYS_OLD` можно убрать. Идемпотентно
- **Резолв в окне ротации**: `blindIndexCandidates()` — поиск по индексам всех
  ключей + плейн-фоллбек, чтобы вебхук/кабинет резолвились до перешифровки
- Новая env `SECRETS_KEYS_OLD`
- Тесты: +5 (старт, окно primary/old, перешифровка, чужой ключ → ошибка, admin) — всего 166
