...
- Не добавлять зависимости без необходимости (сейчас `express`, `dotenv`, `better-sqlite3`; тесты — node:test)
- Не менять схему стейта без миграции — у пользователей живые данные
  (новые поля/таблицы добавлять можно, обрабатывая их отсутствие; пример — core/db.js)
