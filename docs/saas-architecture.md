...
|---|---|---|---|
| **S1. Реестр арендаторов** | `tenants` + `tenant_channels`, `core/tenant.js`, admin-API, seed `default` из env, резолв-сем в вебхуках (пока → default) | низкий (аддитивно) | в работе |
| **S2. Изоляция данных** | `tenant_id` во все таблицы + слой; контекст запроса; тесты изоляции | высокий | план |
