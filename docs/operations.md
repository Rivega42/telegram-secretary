...
- источник (`post_42`) фиксируется в истории лида (SQLite)
- владельцу приходит «🔥 Лид» с источником, репликами и кнопками статуса:
  «✅ В работе / 💰 Продано / ❌ Потерян» (двигаешь воронку в один тап)
- лид сохраняется со статусом; при настроенном `CRM_WEBHOOK_URL` новый лид
  POST-ится во внешнюю CRM (Make/Zapier/Pipedrive)
- список лидов: `GET /api/leads?status=working`; счётчик воронки — в дайджесте
...
## База знаний и петля качества

`persona/facts.md` — факты (услуги, цены, FAQ), на которые секретарь опирается и не
выдумывает сверх них. Пусто — отвечает только из персоны и истории.

Каждый ответ в копии владельцу имеет кнопки 👍/👎 — оценки копятся для качества.
Когда владелец нажимает «🔄 Переписать» и диктует правку — она сохраняется и
**подмешивается как пример** в будущие ответы (стиль/тон владельца). Сводка
правок и оценок — в дайджесте. Выключить few-shot: `FEEDBACK_FEWSHOT=false`.

## Дайджест и метрики

Сводка за сутки приходит владельцу автоматически в `DIGEST_TIME` (МСК) и по команде
`/digest`. Содержит: объём сообщений (вход / ответил секретарь / ответил ты),
активные диалоги, новые контакты, разбивку по платформам, очередь и черновики.

Машиночитаемо — `GET /api/stats?hours=24` (нужен `X-Api-Key`):

```bash
curl -s -H "X-Api-Key: $API_KEY" "localhost:18792/api/stats?hours=24" | jq
```
