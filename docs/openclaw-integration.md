...
Канал (автопостинг)        ─┤                       ┌─ workspace/memory/
Комменты к постам          ─┼─→ secretary-proxy ──→ │  OpenClaw instance(s)
Групповой чат (Q&A)        ─┤    (коннекторы)       └─ MEMORY.md, contacts/
ВКонтакте, WhatsApp, IG    ─┘
```
