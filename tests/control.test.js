...
test('handleControlUpdate: команды чужака не выполняются (он — лид), владелец — обрабатывается', async () => {
  // чужак шлёт команду в личку бота — попадает в лид-воронку, режим НЕ меняется
  setMode('auto');
  await handleControlUpdate(
    { message: { chat: { id: 999, type: 'private' }, from: { id: 999, first_name: 'Гость' }, text: '/off' } }, {});
  assert.equal(getSettings().mode, 'auto');

  // владелец шлёт команду
  await handleControlUpdate(
    { message: { chat: { id: 1, type: 'private' }, from: { id: 1 }, text: '/off' } }, {});
  assert.equal(getSettings().mode, 'off');
  setMode('auto');
});
